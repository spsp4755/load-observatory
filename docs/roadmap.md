...
JSON 배열·JSONL의 도착 간격, 입력 길이, 출력 상한, 시나리오 이름과 선택적 프롬프트를 로컬에서 가져와 기존 Go Agent로 재생한다. 원래 속도와 배속, 최대 진행 요청 제한, 누락 도착 집계, 실행 간 조건 비교를 지원한다. 브라우저 localStorage에는 트레이스를 저장하지 않지만 실행 재현을 위해 Controller 저장소에는 run config로 보관하므로, 민감한 원문은 제거하고 `prompt_tokens`만 반입한다. 세부 형식은 `docs/trace-replay.md`에 기록했다.

## 대상 장애 회복력 (Phase 4) ✅ 완료

**문제**: 부하 테스트 중 사내 모델 서버가 죽으면, VU/RPS 루프는 실패를 정직하게 집계하긴 하지만(TransportErrors/HTTPFailures로 분리, 지연 분포는 오염 안 됨) **재시도를 멈추지 않는다.** closed-loop VU는 연결 거부를 수백~수천 회/초 속도로 재시도하고, 서버가 복구되는 순간 **밀려 있던 모든 VU가 동시에 다시 몰려가서 막 살아난 서버를 즉시 다시 죽일 수 있다.** 부하 테스터가 자신이 측정하려던 장애를 스스로 재생산하는 경로다.

**구현**: `internal/agent/runner.go`에 최소한의 circuit breaker 추가.

- 연속 실패 횟수(`consecutiveFailures`, 실행 전체 공유)가 **3회 이상**이면 다음 시도 전에 지수 백오프(500ms 시작, 30초 상한, ±jitter)로 대기. 성공 시 즉시 리셋.
- **jitter가 핵심**: 같은 시점에 실패한 VU들이 정확히 같은 지연으로 동시에 재시도하면 "몰려가서 죽이는" 문제가 그대로 재현된다. 지연을 무작위로 흩어서 서버가 살아났을 때 요청이 한 번에 몰리지 않게 한다.
- 대기 시간은 `generator_delay`로 집계된다 — 서버가 늦게 응답한 게 아니라 **발생기가 의도적으로 기다린 것**이므로 그 분류가 정확하다.
- 결과에 `BackoffEvents`/`BackoffSeconds`를 노출해 (`export.json/csv`의 `lifecycle` 섹션 포함) "완료율이 낮은 이유가 서버 용량 부족이 아니라 회복 대기였다"를 구분할 수 있게 했다.

**동시에 고친 것 — HTTP 커넥션 풀 기본값**: Go `http.Client{}`의 기본 transport는 호스트당 idle 커넥션을 **2개로 제한**한다. 부하 테스트는 한 호스트(사내 모델 서버)에 수십~수백 VU를 동시에 보내는 워크로드라서, 이 기본값이면 대부분의 요청이 TCP를 매번 재수립하며 — 실제 클라이언트라면 안 낼 오버헤드를 지연시간에 섞어 넣고, 발생기 쪽 ephemeral port 고갈 위험도 만든다. `MaxIdleConnsPerHost`를 2048로 올려 해결.

**실측 검증**: 항상 503을 반환하는 가짜 서버에 VU=1·2초를 걸었을 때, 백오프 없이는 로컬 루프백 기준 초당 수만 건이 나갈 수 있는 환경에서 **50회 미만**으로 억제됨을 확인(`TestDeadTargetIsNotHammeredForTheWholeRun`). 동일 실패 횟수에서 여러 goroutine이 동시에 `backoffDelay`를 호출해도 지연 값이 서로 갈라짐을 확인(`TestJitterSpreadsOutSimultaneousRetries`).

### 다음으로 살펴볼 실무 문제 (미착수)

이번에 코드를 훑으며 함께 발견했지만 이번 변경에는 포함하지 않은 것들:

- **컨트롤러에 접근 통제가 없음.** 네트워크로 닿을 수 있는 누구나 대상 등록·실행·이력 열람이 가능하다. 여러 팀이 폐쇄망에서 같은 인스턴스를 공유하면 "누가 이 부하를 걸었는지" 감사가 안 된다.
- **단일 요청이 걸려도(hang) 실행 종료 시점까지는 계속 "진행 중"으로 잡혀 있음.** 서버가 죽지 않고 응답만 영원히 안 주는 경우, 그 요청은 `DurationSeconds + DrainSeconds`가 끝나야 정리된다. 지금은 정확성엔 문제 없지만(취소로 정직하게 집계됨), 그 VU 슬롯이 그동안 사실상 비생산적으로 묶여 있다는 뜻이다. 개별 요청에 대한 타임아웃을 두려면 "합리적인 길이"를 설정으로 받아야 하는데, 이 도구는 20K+ 토큰·5분 이상 요청을 정상 워크로드로 취급하므로 고정값을 넣을 수 없다 — 설계가 더 필요하다.
- **CI 파이프라인 없음.** `go test`/`go vet`/웹 테스트가 로컬에서만 실행된다.
