...
export function recommendWorkload(kind, form) {
  if (kind === "rag") {
    ...
    return { ...form, agentWorkflow: false, mode: "vu", prompt, maxTokens: "8192", warmupRequests: form.warmupRequests || "3", scenario: [{ name: "RAG 질의", prompt, weight: 1, max_tokens: 8192, think_time_millis: 1200 }] };
  }
  if (kind === "long-agent") {
    return { ...form, agentWorkflow: true, mode: "vu", warmupRequests: form.warmupRequests || "3", scenario: [
  ...
  return { ...form, agentWorkflow: false, mode: "vu", prompt: simplePrompt, maxTokens: "1280", warmupRequests: form.warmupRequests || "3", scenario: [{ name: "간단 질의", prompt: simplePrompt, weight: 1, max_tokens: 1280, think_time_millis: 500 }] };
}
