export function toRunConfig({ targetId, mode, vus, rps, duration, prompt, maxTokens, maxErrorPercent, maxP95Millis, cachePolicy = "mixed", variationPercent = "30", shards = "3", maxTTFTP95Millis = "0", minOutputTokensPerSecond = "0", maxTTPOTP95Millis = "0", minGoodputPercent = "0", maxInFlight = "0", warmupRequests = "0", cooldownSeconds = "0", drainSeconds = "0", steadyStateSeconds = "0", minCompletionPercent = "0", ignoreEOS = false, maxNumSeqs = "0", maxNumBatchedTokens = "0", tensorParallelSize = "0", accumulateContext = false, stages = [], scenario = [], agentWorkflow = false, journeys = [] }) {
	const config = { target_id: targetId, mode, duration_seconds: Number(duration), prompt, max_tokens: Number(maxTokens), max_error_percent: Number(maxErrorPercent), max_p95_millis: Number(maxP95Millis), cache_policy: cachePolicy, variation_percent: Number(variationPercent), shards: Number(shards), max_ttft_p95_millis: Number(maxTTFTP95Millis), min_output_tokens_per_second: Number(minOutputTokensPerSecond), max_tpot_p95_millis: Number(maxTTPOTP95Millis), min_goodput_percent: Number(minGoodputPercent), max_in_flight: Number(maxInFlight), warmup_requests: Number(warmupRequests), cooldown_seconds: Number(cooldownSeconds), drain_seconds: Number(drainSeconds), steady_state_seconds: Number(steadyStateSeconds), min_completion_percent: Number(minCompletionPercent), ignore_eos: Boolean(ignoreEOS), server: { max_num_seqs: Number(maxNumSeqs), max_num_batched_tokens: Number(maxNumBatchedTokens), tensor_parallel_size: Number(tensorParallelSize) }, accumulate_context: Boolean(accumulateContext), stages, scenario, agent_workflow: agentWorkflow, journeys };
