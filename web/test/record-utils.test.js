...
const runs = [
  { id: "run-1", status: "completed", config: { target_id: "model", vus: 5, max_error_percent: 2, max_p95_millis: 2000 }, result: { total: 10, successes: 10, latency: { p95_millis: 100 }, throughput_rps: 5, tokens: { output_per_second: 20 } } },
  { id: "run-2", status: "completed", config: { target_id: "model", vus: 10, max_error_percent: 2, max_p95_millis: 2000 }, result: { total: 10, successes: 9, failures: 1, latency: { p95_millis: 3000 }, throughput_rps: 8, tokens: { output_per_second: 15 } } },
];
...
test("compares output token speed", () => assert.equal(compareRuns(runs[0], runs[1]).find((row) => row.key === "output_per_second").delta, -5));
